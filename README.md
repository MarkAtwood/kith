...
JMAP (RFC 8620 core) with custom capability `urn:ietf:params:jmap:chat`. JSON over HTTPS, listener bound to tailnet interface only. The same endpoint serves both owner clients and peer mailboxes; they differ only in which methods are authorized.
