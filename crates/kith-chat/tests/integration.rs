...
async fn test_full_chat_workflow() {
    let owner_id = "uid-owner-chat";
    let store = make_store();
    let d = make_dispatcher(Arc::clone(&store), owner_id);
